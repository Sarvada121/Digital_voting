    ...
    if (request.method == 'POST'):
        ...
        if (User.objects.filter(username=voterid)).exists():
            ...
            if Voters.objects.filter(voterid_no=voterid):
                ...
                url = "http://2factor.in/API/V1/7e825d24-XXXX-XXXX-XXXX-0200cd93604211/SMS/" + user_phone + "/AUTOGEN"
...
def otp(request):
    if (request.method == "POST"):
        ...
        url = "http://2factor.in/API/V1/7e825d24-XXXX-XXXX-XXXX-0200cd93604211/SMS/VERIFY/" + request.session[
    ...
    if request.method == "POST":
        ...
        if vote.v.has_voted == 'no':
            ...
            if flag==1:
                ...
                url = "http://2factor.in/API/V1/7e825d24-XXXX-XXXX-XXXX-0200cd93604211/SMS/" + vmobno + "/AUTOGEN"
                response = requests.request("GET", url)
                data = response.json()
                request.session['otp_session_data'] = data['Details']
                response_data = {'Message': 'Success'}
...
def subvoteotp(request):
    if (request.method == "POST"):
        userotp = request.POST['otp']
        url = "http://2factor.in/API/V1/7e825d24-XXXX-XXXX-XXXX-0200cd93604211/SMS/VERIFY/" + request.session['otp_session_data'] + "/" + userotp
        response = requests.request("GET", url)
        data = response.json()
        if data['Status'] == "Success":
            v_id = request.session['v_id']
